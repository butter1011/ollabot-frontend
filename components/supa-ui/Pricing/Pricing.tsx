...
export default function Pricing({ products, subscription, user, lang }: Props) {
  ...
  const billingIntervalLocale = {
    month: lang === 'fr' ? 'mois' : 'month',
    year: lang === 'fr' ? 'année' : 'year',
  }

  const subscriptionNameLocale = {
    Starter: lang === 'fr' ? 'Essentiel' : 'Starter',
    Pro: 'Pro',
    Elite: 'Elite', 
    Enterprise: lang === 'fr' ? 'Entreprise' : 'Enterprise',
  }

  const buttonTextLocale = {
    subscribe: lang === 'fr' ? "S'abonner" : 'Subscribe',
    manage: lang === 'fr' ? "S'abonner" : 'Manage',
  }

  const [priceIdLoading, setPriceIdLoading] = useState<string>()
  const currentPath = usePathname()

  const handleStripeCheckout = async (price: Price, metadata: { [key: string]: string } = {}) => {
  ...
  const getFeaturesByLang = (features: Features, lang: string) => {
    return lang === 'fr'
      ? features.locale.fr.features
      : features.locale.en.features
  }
  ...
  if (products.length === 0) {
    ...
    return (
      <section className="container flex w-full flex-col gap-6 bg-pricingBackground py-2 md:py-12 lg:py-24">
            ...
            <div className="relative mt-6 flex self-center rounded-lg border border-white bg-zinc-900 p-0.5 sm:mt-8">
              {intervals.includes('month') && (
                <button
                  ...
                  } m-1 h-11 whitespace-nowrap rounded-md px-8 text-sm font-medium focus:z-10 focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-opacity-50 sm:w-auto sm:px-8`}
            ...
            {products.map((product, index) => {
              ...
              return (
                <div
                  key={product.id}
                  className={cn(
                    'mt-6 flex flex-col divide-y divide-zinc-600 rounded-lg border border-white bg-zinc-900 shadow-sm',
                    {
                ...
                >
                  <div className="p-6">
                    <h2 className="text-center text-2xl font-bold leading-6 text-secondary">
                      {subscriptionNameLocale[product.name]}
                    </h2>
                    ...
                    <span className="text-base font-medium">
                      /{billingIntervalLocale[billingInterval]}
                    </span>
                    </p>
                    <div className="mt-4">
                      ...
                      <ul className="mb-4 flex min-h-0 flex-col gap-3 text-sm font-bold text-grayColor">
                        {productLocaleFeatures[lang].features.map(
                          (feature, idx) => (
                            <li
                              key={idx}
                    ...
                    <Button
                      ...
                      size="lg"
                      className="w-full"
                      onClick={() => handleStripeCheckout(price, { promotekit_referral: window.promotekit_referral })} // Pass metadata here
                      type="button"
                    >
                      {subscription ? buttonTextLocale.manage : buttonTextLocale.subscribe}
                    </Button>
                  </div>
                </div>
              )
        ...
        <div className="mx-auto mb-10 flex w-full max-w-[58rem] flex-col rounded-lg border-4 border-violetColor bg-blackColor  p-8">
          <h3 className="text-md mb-2 font-bold text-white sm:text-xl">
            {lang === 'fr'
              ? 'Vous avez besoin de quelque chose de spécifique ?'
              : 'Need something specific?'}
          </h3>
          <p className=" text-sm leading-normal text-white sm:leading-7">
            <strong>
              {lang === 'fr'
                ? "Nous comprenons que chaque entreprise est unique. C'est pourquoi nous proposons des plans personnalisés, vous assurant de ne payer que pour les fonctionnalités dont vous avez besoin. Prêt pour une solution logicielle sur mesure ? Réservez une réunion avec nous ici."
                : "We understand that every business is unique. That's why we provide personalized plans, ensuring you pay only for the features you require. Ready for a custom software solution? Book a meeting with us here."}
            </strong>
          </p>
          <br />

          <Link href="https://cal.ollabot.com/meeting/custom-ai-chatbot-consultation" className="text-lg text-white font-bold hover:underline">
            {lang === 'fr'
              ? 'Réservez une réunion avec nous ici →'
