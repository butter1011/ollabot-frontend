...
const queryPineconeVectorStore = async (
  ...
  try {
    ...
    matches = matches.filter(
      (match: { score: number; metadata: { text: string | Array<any> } }) =>
        match.score > 0.82 &&
    ...
    const uniqueMatches = matches.reduce(
      (acc: Array<any>, current: { metadata: { text: string } }) => {
    ...
    while (formattedResults.length > 7500) {
      const lastContextIndex = formattedResults.lastIndexOf('[CONTEXT ')
