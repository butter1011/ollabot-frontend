...
export function DashboardSidebar({ items, subscription }: DashboardNavProps) {
  const isActiveSubscription = subscription && ["trialing", "active"].includes(subscription.status);
  ...
  return (
    <nav className="grid items-start gap-2">
      {items.map((item, index) => {
        ...
        const isDisabled = (item.requiresActiveSubscription && !isActiveSubscription) || item.disabled;
