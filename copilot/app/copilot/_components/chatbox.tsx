...
const Chatbox = ({
  ...
  }, [chatbotName, avatarUrl, ai_message, welcomeMessage])
  ...
  const handleSendMessage = async () => {
    ...
    if (message.trim()) {
      ...
      try {
        ...
        const response = await fetch(`${domain}/api/chatbot/${botId}/chatPublic`, {
          method: 'POST',
          body: formData,
        })
        ...
        {messages.map((msg, index) => (
          <div key={index} className="mt-3 flex gap-1 space-x-2">
            ...
            <div className="ml-2 w-full grid-rows-2 text-wrap">
              ...
              <div
                className={`prose overflow-y-auto break-normal rounded-lg bg-accent p-2 ${msg.isThinking ? 'animate-pulse' : ''}`}
              >
                <ReactMarkdown
        ...
        <div className="ml-3 mt-3 flex w-full justify-center">
          {watermark ? (
            <div className="flex items-center gap-2 text-sm font-medium">
              <a href="https://ollabot-frontend-nine.vercel.app" target="_blank" rel="noopener noreferrer">
                Built with Ollabot
              </a>
              <img
                alt="Ollabot"
