...
export const useRecordVoice = (botId: string) => {
  ...
  const getText = async (base64data: string) => {
    try {
      const response = await fetch(`https://ollabot-frontend-nine.vercel.app/api/chatbot/${botId}/transcribe`, {
      // const response = await fetch(`http://localhost:3000/api/chatbot/${botId}/transcribe`, {
        method: 'POST',
        headers: {
